package tech.dhondt.neo4j.playground.uitls;
...
final public class InMemoryCompiler {
    ...
    public Object run(final String className, final String methodName, Class<?>... parameterTypes) throws IllegalAccessException, ClassNotFoundException, NoSuchMethodException, InvocationTargetException {

        if (!valid) {
            return null;
        }
        final Class<?> theClass = getCompiledClass(className);
        final Method mainMethod = theClass.getDeclaredMethod(methodName, parameterTypes);
        return mainMethod.invoke(null);
    }
